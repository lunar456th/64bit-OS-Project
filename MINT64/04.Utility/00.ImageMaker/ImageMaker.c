...
int AdjustInSectorSize(int iFd, int iSourceSize); // fill from current position to times of 512 bytes with zero.
void WriteKernelInformation(int iTargetFd, int iTotalKernelSectorCount, int iKernel32SectorCount); // insert info about kernel into bootloader
int CopyFile(int iSourceFd, int iTargetFd); // copy src to dest and return the size copied.
...
int main(int argc, char * argv[])
{
	...
	// open bootloader and copy all contents into disk image file
	...
	// open kernel32 and copy all contents into disk image file
	...
	// update kernel info into disk image
...
int AdjustInSectorSize(int iFd, int iSourceSize)
{
	...
	iAdjustSizeToSector = iSourceSize % BYTES_OF_SECTOR;
	cCh = 0x00;
	...
	{
		printf("[INFO] File size if aligned 512 byte\n");
	}

	iSectorCount = (iSourceSize + iAdjustSizeToSector) / BYTES_OF_SECTOR;
	return iSectorCount;
}
